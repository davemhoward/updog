...
geno<-BEDMatrix(paste0("ukb/ukb_chr",chr,"_",sprintf("%03d",chunk)))
bim<-read.table(paste0("ukb/ukb_chr",chr,"_",sprintf("%03d",chunk),".bim"),header=F,sep="")
fam<-read.table(paste0("ukb/ukb_chr",chr,"_",sprintf("%03d",chunk),".fam"),header=F,sep="")
##phenotype<-read.table("BroadDepressionPhenotypeforMeta.txt",header=F,sep="")

## Pull in ld files for 1000G ##
ld<-BEDMatrix(paste0("~/scratch/g1000_eur_chr",chr,"_",sprintf("%03d",chunk)))
ldbim<-read.table(paste0("~/scratch/g1000_eur_chr",chr,"_",sprintf("%03d",chunk),".bim"),header=FALSE,sep="",stringsAsFactors=F)
ldbim$ld<-0
window<-250000

## Pull in original summary statistics
sumstats<-read.table(paste0("23andmePGC_sumstats_chr",chr,"_",sprintf("%03d",chunk),".ma"),header=T,sep="",stringsAsFactors=F)

## Pull in PRScs scores for chromosome
prscs<-read.table(paste0("chunkscores_chr",chr,"_",sprintf("%03d",chunk)),header=F,sep="")
## Check rsnumber is V2, A1 is in V4 and beta is in V6 
