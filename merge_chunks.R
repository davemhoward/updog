...
for (n in 1:(length(chunkfiles))) {
  if ((file.exists(scores[n])) == FALSE) {
    ...
    sbatchargs<-paste0("--export=ALL,i=",chr,",testloc=",args[1],",testtype=",args[2],",ldloc=",args[3],",ldtype=",args[4],",sumstats=",args[5],",scores=",args[6],",plinkloc=",args[7],",rloc=",args[8],",outname=",args[9],",sumstatsOR=",args[10],",jobargs=",args[11])
...
if (p >= 1) {
  ...
  sink(paste0("../resubmitjobs_",name),append=TRUE)
  mergeargs<-paste0("--export=ALL,testloc=",args[1],",testtype=",args[2],",ldloc=",args[3],",ldtype=",args[4],",sumstats=",args[5],",scores=",args[6],",plinkloc=",args[7],",rloc=",args[8],",outname=",args[9],",sumstatsOR=",args[10],",jobargs=",args[11])
